...
def worker_get_density_estim_data(seeds_list, h5_path, metrics):
    with h5py.File(h5_path, "r") as f:
        ...
        for metric in metrics:
            if metric in ["prate_sqw_e", "prate_sqw_i"]:
                aux_dt.append( ( metric, np.float64, ( int( (f[keys[0]].attrs["t_stop_rec"]-f[keys[0]].attrs["t_start_rec"])/f[keys[0]].attrs["bin_size_big"] )-1 ) ) )
            else:
...
def get_density_estim_data(h5_path: str, metrics: List[str], parallel=False, n_workers=2):
    ...
    with h5py.File(h5_path, "r") as f:
        ...
        for metric in metrics:
            if metric in ["prate_sqw_e", "prate_sqw_i"]:
                aux_dt.append( (metric, np.float64, ( int( (sim_params["t_stop_rec"]-sim_params["t_start_rec"])/sim_params["bin_size_big"] )-1 ) ) )
            else:
                aux_dt.append((metric, np.float64))
        dt = np.dtype(aux_dt)
