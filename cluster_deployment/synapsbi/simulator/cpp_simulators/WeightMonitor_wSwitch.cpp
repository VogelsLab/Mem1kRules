...
WeightMonitor_wSwitch::WeightMonitor_wSwitch(SparseConnection * source, std::string filename, AurynDouble interval ) : Monitor(filename)
{
	init(source,0,0,filename,interval/auryn_timestep);
}

WeightMonitor_wSwitch::WeightMonitor_wSwitch(SparseConnection * source, ForwardMatrix * m, std::string filename, AurynDouble interval ) : Monitor(filename)
{
	init(source,0,0,filename,interval/auryn_timestep);
	set_mat(m);
}

WeightMonitor_wSwitch::WeightMonitor_wSwitch(SparseConnection * source, NeuronID i, NeuronID j, std::string filename, AurynDouble interval, RecordingMode mode, StateID z ) : Monitor(filename)
{
...
WeightMonitor_wSwitch::~WeightMonitor_wSwitch()
{
	delete element_list;

}

void WeightMonitor_wSwitch::init(SparseConnection * source, NeuronID i, NeuronID j, std::string filename, AurynTime stepsize)
{
...
void WeightMonitor_wSwitch::add_to_list_by_data_index(AurynLong data_index, StateID z)
{
	element_list->push_back( data_index + z*mat->get_statesize() );
}

void WeightMonitor_wSwitch::add_to_list(AurynWeight * ptr)
{
	if ( ptr != NULL ) {
...
void WeightMonitor_wSwitch::add_to_list(NeuronID i, NeuronID j, StateID z)
{
	if ( mat->exists(i, j, z) ) {
		add_to_list_by_data_index( mat->get_data_index(i, j), z );
	} else {
		std::stringstream oss;
		oss << "WeightMonitor_wSwitch:: Tried adding element " 
			<< i << ", " 
			<< j << " " 
			"z=" << z << " " 
...
void WeightMonitor_wSwitch::add_to_list( std::vector<neuron_pair>  vec, std::string label )
{
	if ( recordingmode == ELEMENTLIST || recordingmode == GROUPS ) {
		std::stringstream oss;
		oss << "WeightMonitor_wSwitch:: Adding " << vec.size() << " elements to index list " << label;
		auryn::logger->msg(oss.str(),VERBOSE);
...
void WeightMonitor_wSwitch::add_equally_spaced(NeuronID number, NeuronID z)
{
	if ( z >= mat->get_num_z_values() ) {
		auryn::logger->msg("WeightMonitor_wSwitch:: z too large. Trying to monitor complex "
	...
	if ( number > src->get_nonzero() ) {
		auryn::logger->msg("WeightMonitor_wSwitch:: add_equally_spaced: "
...
void WeightMonitor_wSwitch::load_data_range( AurynLong i, AurynLong j )
{
...
std::vector<type_pattern> * WeightMonitor_wSwitch::load_patfile( std::string filename, unsigned int maxpat )
{
	...
	while( !fin.eof() && patterns->size() < maxpat ) {
		...
		if (line == "") { 
			if ( total_pattern_size > 0 ) {
				std::stringstream oss;
				oss << "WeightMonitor_wSwitch:: Read pattern " 
					<< patterns->size() 
					<< " with pattern size "
					<< total_pattern_size
					<< " ( "
...
void WeightMonitor_wSwitch::load_pattern_connections( std::string filename , unsigned int maxcon, unsigned int maxpat, PatternMode patmod )
{
	load_pattern_connections( filename, filename, maxcon, maxpat, patmod );
}


void WeightMonitor_wSwitch::load_pattern_connections( std::string filename_pre, std::string filename_post , unsigned int maxcon, unsigned int maxpat, PatternMode patmod )
{
	if ( !src->get_destination()->evolve_locally() ) return ;
	...
	std::stringstream oss;
	oss << "WeightMonitor_wSwitch:: Finished loading connections from n_pre=" 
		<< patterns_pre->size() 
		<< " and n_post="
		<< patterns_post->size() 
		<< " patterns";
...
void WeightMonitor_wSwitch::record_single_synapses()
{
...
void WeightMonitor_wSwitch::record_synapse_groups()
{
...
void WeightMonitor_wSwitch::execute()
{
    if ( !active ) return;
...
void WeightMonitor_wSwitch::set_mat(ForwardMatrix * m)
{
	mat = m;
}

void WeightMonitor_wSwitch::turn_on()
{
    active = true;
}

void WeightMonitor_wSwitch::turn_off()
{
    active = false;
}