...
*
* This is a copy paste of the WeightMonitor class from default Auryn but with 2 added functions to turn on and off the monitor (protected inheritance otherwise)
*
*/

#ifndef WEIGHTMONITOR_WSWITCH_H_
#define WEIGHTMONITOR_WSWITCH_H_
 ...
 * synapses TURNED OFF AS ALREADY DEFINED IN NATIVE AURYN IN WEIGHTMONITOR*/
// enum RecordingMode { 
 ...
 * whereas ASSEMBLIES_ONLY restricts recording to inside each pattern. TURNED OFF AS ALREADY DEFINED IN NATIVE AURYN IN WEIGHTMONITOR */
...
class WeightMonitor_wSwitch : protected Monitor
{
protected:
...
public:
	 ...
	 * */
	WeightMonitor_wSwitch(SparseConnection * source, string filename, AurynDouble interval=10.0);
	 ...
	 * \param interval The sampling interval in simulation interval in seconds
	 * */
	WeightMonitor_wSwitch(SparseConnection * source, ForwardMatrix * m, string filename, AurynDouble interval=10.0);
	 ...
	 * */
	WeightMonitor_wSwitch(SparseConnection * source, NeuronID i, NeuronID j, string filename, AurynDouble interval=10.0, RecordingMode mode = SINGLE, StateID z = 0);

	virtual ~WeightMonitor_wSwitch();
    ...
    /*! \brief access the active field from parent class Monitor which is protected */
    void turn_on();
    void turn_off();
};
...
#endif /*WEIGHTMONITOR__WSWITCH_H_*/